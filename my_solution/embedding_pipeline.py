...
class TranscriptProcessor:
    def __init__(self, acronym_mapping: Dict[str, str]):
        # Our regex from before, optimized for performance
        self.pattern = re.compile(
            r"(?P<timestamp>\d{3}:\d{2}:\d{2})\s+(?P<speaker>[A-Z]{2,4})\n(?P<content>.*?)(?=\n\d{3}:\d{2}:\d{2}|$)",
            re.DOTALL
        )
        self.expander = AcronymExpander(acronym_mapping)

    def process_transcript(self, raw_text: str, metadata: Dict[str, Any]) -> Generator[Dict[str, Any], None, None]:
        """
        ...
        for match in self.pattern.finditer(raw_text):
            ...
            enriched_speaker = self.expander.process_text(speaker)
...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def get_metadata_log_prefix(self, metadata) -> str:
        prefix = f"[file({metadata.get('file_path', 'unknown')})] [data_type({metadata.get('data_type', 'unknown')})] [mission({metadata.get('mission', 'unknown')})]"
        return prefix
    ...
    def get_acronym_lookup(self, text: str, model: str = "gpt-3.5-turbo") -> List[Dict[str, Any]]:
        """
...
"""
    ...
    def parse_transcript_document(self, text: str, metadata: Dict[str, Any]) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        title_text, intro_text, acronym_text, log_text = self.parse_transcript_sections(text)
